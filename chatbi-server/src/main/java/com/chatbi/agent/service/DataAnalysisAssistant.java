...
public interface DataAnalysisAssistant {
    ...
    @SystemMessage("""
            ...
            1. 先调用 schema_search 工具，传入与用户问题相关的关键词，获取数据库表结构
            2. 根据返回的表结构，生成正确的 PostgreSQL SELECT 查询语句
            3. 调用 execute_sql 工具执行查询
            ...
            - 如果无法确定答案，诚实��知用户
            - schema_search 最多调用 2 次，如果 2 次都未找到相关表，直接告知用户当前项目没有匹配的数据表，不要继续重试
            """)
    String chat(@MemoryId String memoryId, @UserMessage String message, InvocationParameters parameters);
}
